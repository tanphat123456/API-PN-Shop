...
export class HandleBase64 {
    ...
    static generateApiKey(api_key: string): string {
        ...
        if (!api_key) {
            throw new HttpException(new ExceptionResponseDetail(HttpStatus.FORBIDDEN, "api_key không hợp lệ!"), HttpStatus.OK);
        } else {
    ...
    static async decodePasswordBase64(password: string): Promise<string> {
        if (!password) 
        {
            throw new HttpException(new ExceptionResponseDetail(HttpStatus.BAD_REQUEST, "password không được rỗng!"), HttpStatus.OK);
        } else {
    ...
    static decodeSecretKey(api_key: string): string {
        if (!api_key) {
            throw new HttpException(new ExceptionResponseDetail(HttpStatus.BAD_REQUEST, "api_key không hợp lệ!"), HttpStatus.OK);
        } else {
